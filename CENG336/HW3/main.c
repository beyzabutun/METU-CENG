...
/* Global Variables*/
unsigned int RB4Pressed=0, timer1Interrupted=0, timer0Interrupted=0, ADCInterrupted=0, passed10ms=0, RB4bounced=0;
...
void __interrupt() isr(void){
    ...
    if(RBIF==1){
      ...
      if((portb_read & 16) == 16){
         if(RB4bounced && passed10ms){
            RB4Pressed = 1;
            rb4_handled();		//TODO : debouncing??
         }
        else if(RB4bounced && passed10ms == 0){
          // start tmr3 again
          TMR3ON = 0;
          TMR3 = 53036;
          TMR3IE = 1;
...
void timer1Interrupt(void){
    ...
    */
    ...
    if(gameFinished==0){
        ...
        if(timer1Value % 10 == 0){      //NOT SURE WE SHOULD DO IT!!!
          hs_passed();
        }
